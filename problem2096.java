...
class problem2096{
    ...
    public String getDirections(TreeNode root, int startValue, int destValue) {
        ...
        int k=commonPathLen;
        for (int i = 0; i < pathToDestination.size()-commonPathLen; i++) {
            res.append(pathToDestination.get(k));
            k++;
        }
        return res.toString();
    }
    ...
    public boolean findPath(TreeNode curNode,int targetValue,List<String> pathToAppend){
        if(curNode==null){
            return false;
        }
        if(curNode.val==targetValue){
        ...
        pathToAppend.add("R");
        if(findPath(curNode.right, targetValue, pathToAppend)){
            return true;
        }
        pathToAppend.remove(pathToAppend.size()-1);
        pathToAppend.add("L");
        if(findPath(curNode.left, targetValue, pathToAppend)){
            return true;
        }
        pathToAppend.remove(pathToAppend.size()-1);
        return false;
